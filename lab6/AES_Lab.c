#include <stdio.h>
#include <stdlib.h>
#include <string.h>

unsigned char sub[16][16] = {};

unsigned char mix[4][4] = {{1, 4, 4, 5},
                           {5, 1, 4, 4},
                           {4, 5, 1, 4},
                           {4, 4, 5, 1}};

unsigned char invmix[4][4] = {{165, 7, 26, 115},
                              {115, 165, 7, 26},
                              {26, 115, 165, 7},
                              {7, 26, 115, 165}};

void subbytes(unsigned int pt[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            int ii = pt[i][j] >> 4;
            int jj = pt[i][j] & 0b1111;
            pt[i][j] = sub[ii][jj];
        }
    }
}

void shiftRows(unsigned int pt[4][4]) {
    unsigned int temp[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            temp[i][j] = pt[i][j];
        }
    }

    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            pt[i][j] = temp[i][(j + i) % 4];
        }
    }
}

unsigned int x4(unsigned int x) {
    unsigned int msb1 = x >> 7;
    unsigned int msb2 = x >> 6;

    x = x << 2;
    if (msb1 == 1) x ^= 0b1000110110;
    if (msb2 == 1) x ^= 0b100011011;

    return x;
}

unsigned int xy(unsigned int x, unsigned int y) {
    if (y == 4) return x4(x);
    if (y == 5) return x4(x) ^ x;
}

void mixColumns(unsigned int pt[4][4]) {
    unsigned int temp[4][4];
    for (int i = 0; i < 4; i++) 
        for (int j = 0; j < 4; j++) 
            temp[i][j] = pt[i][j];

    for (int i = 0; i < 4; i++) 
        for (int j = 0; j < 4; j++) 
            pt[i][j] = xy(temp[0][j], mix[i][0]) ^ xy(temp[1][j], mix[i][1]) ^ xy(temp[2][j], mix[i][2]) ^ xy(temp[3][j], mix[i][3]);
    
}



void AESEncrypt(unsigned int pt[4][4], unsigned int key[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            unsigned int msb = pt[i][j] >> 7;
            pt[i][j] = (pt[i][j] << 1) ^ 1;
            if (msb == 1) pt[i][j] ^= 0b100011011;
        }
    }

    subbytes(pt);

    printf("Subbytes: \n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
            printf("%02x ", pt[i][j]);
        printf("\n");
    }

    printf("\n");
    shiftRows(pt);

    printf("ShiftRows: \n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
            printf("%02x ", pt[i][j]);
        printf("\n");
    }

    printf("\n");
    mixColumns(pt);

    printf("MixColumns: \n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
            printf("%02x ", pt[i][j]);
        printf("\n");
    }

    printf("\n");
}

void main() {
    unsigned int pt[4][4], key[4][4];

    // Test Plaintext: 00 11 22 33 44 55 66 77 88 99 aa bb cc dd ee ff
    // Test Key: 00 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f

    printf("Enter the plaintext: ");
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            scanf("%x", &pt[i][j]);

    printf("Enter the key: ");
    for (int i = 0; i < 4; i++) 
        for (int j = 0; j < 4; j++)
            scanf("%x", &key[i][j]);

    printf("Plaintext: \n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) 
            printf("%02x ", pt[i][j]);
        printf("\n");
    }

    printf("\n");

    AESEncrypt(pt, key);

    printf("Encrypted: \n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
            printf("%02x ", pt[i][j]);
        printf("\n");
    }
}